 ...
 */
export async function submitAmbasukeScore(
  ...
  if (payload.game_slug !== 'ambasuke-protocol') {
  ...
  try {
    const { error } = await (supabase.from('minigame_scores') as any).upsert(
      {
        player_name: name,
        game_slug: 'ambasuke-protocol' as const,
    ...
    if (error) {
      console.error('[AmbasukeProtocol] Supabase upsert error:', error)
      return { success: false, error: 'Database error' }
    }

    return { success: true }
  } catch (err) {
    console.error('[AmbasukeProtocol] Unexpected error:', err)
